...
class Quaternion():
    ...
    def __repr__(self):
        string = "Quaternion (wxyz): [{}, {}, {}, {}]".format(self.w, self.x, self.y, self.z)
        return string
...
class GNSSaidedINSwithEKF():
    ################################################
    ### Initialization function for class object ###
    ### Input: None                              ###
    ### Output: None                             ###
    ################################################
    ...
    def predict(self):
        ...
        self.prev_q_est = np.copy(self.q_est)
        #rint(acc_B.T, gyr_B.T, self.gravity.T)
        #rint(self.q_est.T)
        #rint(Quaternion(*self.q_est.reshape(4)).to_mat())
...
##############################################
### Self-explanatory                       ###
### Input: roll/pitch/yaw variables        ###
### Output: np.array of size (3,3)         ###
##############################################
...
######################################
### Self-explanatory               ###
### Input: np.array of size (3,3)  ###
### Output: np.array of size (4,1) ###
######################################
...
### Input: np.array of size (4,1) (a is for debug so del)     ###
### Output: np.array of size (3,3)                            ###
#################################################################
def compRotMatFromQuat(q):
    ...
    #val = (R_BtoN@a) - np.array([[0],[0],[1]])
    #ys.stdout.write('\r{}'.format(val.reshape((1,3))))
    #ys.stdout.flush()
    return R_BtoN

######################################
### Self-explanatory               ###
### Input: np.array of size (4,1)  ###
### Output: np.array of size (3,1) ###
######################################
...
##############################################
### Self-explanatory                       ###
### Input: dict with roll/pitch/yaw fields ###
### Output: np.array of size (4,1)         ###
##############################################
...
############################################
### To be used for multiple calculations ###
### Input: np.array of size(3,1)         ###
### Output: np.array of size (3,3)       ###
############################################
def getSkewMatrix(v):
...
#######################################################################################################################
#######################################################################################################################
### INITIALIZATIONS AND MAIN FUNCTIONS                                                                              ###
#######################################################################################################################
#######################################################################################################################
        ...
        while True:
            ...
            while True:
                ...
                if imu.IMURead():
                    ...
                    if firstFlag:
                        print('setting initial quaternion with EA, hope for the best - {}'.format(ekf.curIMUdata['fusionQPose']))
                        ekf.prev_q_est = Quaternion(*ekf.curIMUdata['fusionQPose']).to_numpy()
                        firstFlag = False
                    #rint('IMU read, {}'.format(ekf.curIMUdata['fusionQPose']))
                    '''
